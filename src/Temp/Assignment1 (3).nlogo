...
turtles-own [
  ...
  threats             ;; agentset of nearby predators
  nearest-neighbor   ;; closest one of our flockmates
  nearest-predator  ;; closest one of predator
  around-birds             ;; agentset of nearby birds
  nearest-bird  ;; closest one of birds
  weight-target ;; creat a value of weight that can used to decide which prey is the most weakest
]
...
to go
  ;; where is the weakest target in the prey's group.
  ask turtles [
    if color = white [func-searchingTheWeakestprey
			run-away
    			flock]
  ...
  ask predators [
    ;; make the predator Chasing the weakest prey.
    fd 1
  ]
  every 1 [
    ask predators [
      let t max-one-of preys [weight-target]
      ask links [hide-link]
      face max-one-of preys [weight-target]
      create-link-with t
      if ChasingLine = false [ask links [hide-link] ]
    ]
  ]
...
to func-searchingTheWeakestprey
  ;; give a weight for each prey that shows how many preys is arrouding itselfs.
  set weight-target count preys in-radius 100
end
to find-birds
   set around-birds preys in-radius vision ;;find around birds in vision
...
to run-away
  find-predators ;;find if there is any predators around
  if any? threats
  [
    find-nearest-predator  ;;find the nearest one
     if distance nearest-predator < escape-range
    ;;if nearest predator close than eascape range, means the bird must escape, or will be kill, then try to escape
    [escape]
  ]
...
to find-predators
  set threats predators in-radius vision

end

to find-nearest-predator ;; turtle procedure
  set nearest-predator min-one-of threats [distance myself]
end

to escape
    ;;if the bird fly in almost the same direction (deviation within 90 degeree) with the predator
  ;;make the bird turn around
  ;;other direction, eascape to change to the same direction of the predator
  ;;fly with 2 times of the normal speed to eascape
  ;;user can change the deviation angle of heading called eascape-turn


  ;;if (abs (subtract-headings heading [heading] of nearest-predator) )> 100
  ;;turn-away ([heading] of nearest-predator) 90
  ;;set heading ([heading] of nearest-predator) - escape-turn
  ifelse (abs (heading - [heading] of nearest-predator) ) < 90
  [set heading (heading) + 180 + escape-turn
  fd 3]
  [set heading ([heading] of nearest-predator) + escape-turn
    fd 3]
  ;;set heading random [heading] of nearest-predator 359
  ;;right random 0 [heading] of nearest-predator
end
